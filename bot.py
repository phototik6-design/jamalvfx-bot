...
SERVICES = {
    "cover": {
        ...
        "tiers": {
            "cover_1": {
                "name": "🥇 پکیج VIP",
                "desc": "طراحی کاور حرفه‌ای و فضاسازی، انتخاب پالت رنگی اختصاصی، فونت‌آرایی حرفه‌ای، افکت‌گذاری حرفه‌ای",
                "price": "۱۱۰۰ هزار تومان",
                "gift": "🎁 هدیه: طراحی کامینگ‌سون رایگان",
            },
            "cover_2": {
                "name": "🥈 پکیج حرفه‌ای",
                "desc": "طراحی کاور حرفه‌ای، پالت رنگی، افکت‌گذاری، فونت‌آرایی",
                "price": "۸۸۰ هزار تومان",
                "gift": None,
            },
            "cover_3": {
                "name": "🥉 پکیج اقتصادی",
                "desc": "طراحی کاور حرفه‌ای و مناسب کار، پالت رنگی، فونت‌آرایی",
    ...
    "eq": {
        ...
        "tiers": {
            "eq_1": {
                "name": "🥇 پکیج VIP",
                "desc": "طراحی اکولایزر حرفه‌ای و منحصربه‌فرد، موشن‌گرافی حرفه‌ای، افکت‌گذاری حرفه‌ای",
                "price": "۱۵۰۰ تومان",
                "gift": "🎁 هدیه: نسخه‌ی کامینگ‌سون استوری رایگان",
            },
            "eq_2": {
                "name": "🥈 پکیج حرفه‌ای",
                "desc": "طراحی اکولایزر حرفه‌ای، افکت‌گذاری حرفه‌ای",
                "price": "۱۲۰۰ تومان",
                "gift": None,
            },
            "eq_3": {
                "name": "🥉 پکیج اقتصادی",
                "desc": "طراحی اکولایزر حرفه‌ای و مناسب کار",
...
# مراحل مکالمه برای ویرایش سفارش
ASK_EDIT_NAME, ASK_EDIT_PHONE, ASK_EDIT_DETAILS, CONFIRM_EDIT = range(8, 12)
...
def update_order_fields(order_id, fields):
    """ویرایش اطلاعات یک سفارش (نام/شماره/توضیحات)"""
    orders = load_orders()
    if order_id in orders:
        orders[order_id].update(fields)
        orders[order_id]["edited_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        save_orders(orders)
        return True
    return False
...
def order_success_keyboard(order_id):
    """منوی نمایش‌داده‌شده بعد از ثبت موفق سفارش (پیگیری/ویرایش)"""
    keyboard = [
        [InlineKeyboardButton("📦 پیگیری سفارش", callback_data=f"track_{order_id}")],
        [InlineKeyboardButton("✏️ ویرایش سفارش", callback_data=f"edit_order_{order_id}")],
...
async def receive_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    final_message = (
        ...
        f"🆔 شناسه سفارش: `{order.get('order_id')}`\n"
        "می‌تونی وضعیت سفارشت رو پیگیری کنی یا در صورت نیاز ویرایشش کنی:"
    )
    await update.message.reply_text(
        final_message,
        reply_markup=order_success_keyboard(order.get("order_id")),
        parse_mode="Markdown",
    )
    context.user_data.pop("order", None)
    return ConversationHandler.END
...
async def receive_reorder_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    final_message = (
        "🎉 رسید دریافت شد و برای بررسی ارسال شد.\n"
        "به‌زودی از طریق تلگرام یا اینستاگرام باهات هماهنگ می‌کنیم. ممنون 🙏\n\n"
        f"🆔 شناسه سفارش: `{reorder.get('order_id')}`\n"
        "می‌تونی وضعیت سفارشت رو پیگیری کنی یا در صورت نیاز ویرایشش کنی:"
    )
    await update.message.reply_text(
        final_message,
        reply_markup=order_success_keyboard(reorder.get("order_id")),
        parse_mode="Markdown",
    )
...
# --------------------- مکالمه‌ی ویرایش سفارش ---------------------
async def edit_order_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    if order.get("user_id") != query.from_user.id:
        await query.answer("شما اجازه ویرایش این سفارش رو نداری.", show_alert=True)
        return ConversationHandler.END

    if order.get("status") != "pending":
        await query.edit_message_text(
            "این سفارش دیگه قابل ویرایش نیست چون وضعیتش تغییر کرده.\n"
            "برای تغییرات بیشتر می‌تونی مستقیم پیام بدی، هماهنگ می‌کنیم 🙏",
            reply_markup=main_menu_keyboard(),
        )
        return ConversationHandler.END

    context.user_data["edit_order"] = {"order_id": order_id}
    await query.edit_message_text(
        f"✏️ *ویرایش سفارش*\n🆔 `{order_id}`\n\n"
        f"👤 نام فعلی: {order.get('name')}\n\n"
        "نام جدید رو بنویس (یا همون قبلی رو دوباره بفرست):",
        parse_mode="Markdown",
    )
    return ASK_EDIT_NAME


async def ask_edit_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["edit_order"]["name"] = update.message.text
    contact_btn = ReplyKeyboardMarkup(
        [[KeyboardButton("📱 ارسال شماره من", request_contact=True)]],
        one_time_keyboard=True,
        resize_keyboard=True,
    ...
    await update.message.reply_text(
        "شماره تماس جدید رو بفرست (یا همون قبلی رو دوباره بنویس):",
        reply_markup=contact_btn,
    )
    return ASK_EDIT_PHONE


async def ask_edit_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.contact:
        phone = update.message.contact.phone_number
    else:
        phone = update.message.text
    context.user_data["edit_order"]["phone"] = phone

    await update.message.reply_text(
        "توضیحات جدید رو بنویس (یا همون قبلی رو دوباره بفرست):",
        reply_markup=ReplyKeyboardRemove(),
    )
    return ASK_EDIT_DETAILS


async def confirm_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["edit_order"]["details"] = update.message.text
    edit = context.user_data["edit_order"]

    summary = (
        "📋 *خلاصه ویرایش سفارش*\n\n"
        f"🆔 شناسه: `{edit['order_id']}`\n"
        f"👤 نام: {edit['name']}\n"
        f"📞 شماره: {edit['phone']}\n"
        f"📝 توضیحات: {edit['details']}\n\n"
        "این تغییرات رو ثبت کنم؟"
    )
    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("✅ بله، ثبت کن", callback_data="confirm_edit_yes")],
            [InlineKeyboardButton("❌ انصراف", callback_data="confirm_edit_no")],
        ]
    )
    await update.message.reply_text(summary, reply_markup=keyboard, parse_mode="Markdown")
    return CONFIRM_EDIT
...
async def finalize_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()

    if query.data == "confirm_edit_no":
        await query.edit_message_text("ویرایش لغو شد.", reply_markup=main_menu_keyboard())
        context.user_data.pop("edit_order", None)
        return ConversationHandler.END

    edit = context.user_data.get("edit_order", {})
    order_id = edit.get("order_id")

    update_order_fields(order_id, {
        "name": edit.get("name"),
        "phone": edit.get("phone"),
    ...
    admin_text = (
        "✏️ *سفارش ویرایش شد*\n\n"
        f"🆔 شناسه: `{order_id}`\n"
        f"👤 نام جدید: {edit.get('name')}\n"
        f"📞 شماره جدید: {edit.get('phone')}\n"
        f"📝 توضیحات جدید: {edit.get('details')}\n\n"
        f"🔗 آیدی تلگرام مشتری: @{user.username if user.username else user.id}"
    )
    try:
        await context.bot.send_message(chat_id=ADMIN_CHAT_ID, text=admin_text, parse_mode="Markdown")
    except Exception as e:
        logger.error("ارسال ویرایش سفارش به ادمین با خطا: %s", e)

    await query.edit_message_text(
        f"✅ سفارش `{order_id}` با موفقیت ویرایش شد.\n\nمی‌تونی وضعیتش رو پیگیری کنی:",
        reply_markup=order_success_keyboard(order_id),
        parse_mode="Markdown",
    )
    context.user_data.pop("edit_order", None)
    return ConversationHandler.END
...
def main():
    ...
    edit_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(edit_order_start, pattern=r"^edit_order_")],
        states={
            ASK_EDIT_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, ask_edit_phone)],
            ASK_EDIT_PHONE: [MessageHandler((filters.TEXT | filters.CONTACT) & ~filters.COMMAND, ask_edit_details)],
            ASK_EDIT_DETAILS: [MessageHandler(filters.TEXT & ~filters.COMMAND, confirm_edit)],
            CONFIRM_EDIT: [CallbackQueryHandler(finalize_edit, pattern=r"^confirm_edit_")],
        },
        fallbacks=[
            CommandHandler("cancel", cancel),
            CommandHandler("start", start),
